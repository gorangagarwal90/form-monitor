...
def card_for(img):
    ...
    for _ in range(8):
        ...
        if SKU_RE.search(txt) or ('View Details' in txt and re.search(r'\$\s*[\d,.]+',txt)):
            return node
    return img.parent
...
def extract_products(html,base):
    ...
    for img in soup.find_all('img'):
        ...
        # Product cards must carry commerce text or a product-specific alt/SKU.
        if not sku and not re.search(r'\$\s*[\d,.]+',txt):continue
        key=sku or purl
        # Keep the first non-thumbnail source per product.
        out.setdefault(key,{'sku':sku,'product_url':purl,'title':alt,'image_url':iu,'card_text':txt[:500]})
    return out,soup.get_text(' ',strip=True)

def fetch(sess,u):
    r=sess.get(u,timeout=60,headers={'User-Agent':UA,'Accept':'text/html,application/xhtml+xml','Referer':'https://www.gemsny.com/'})
    r.raise_for_status()
    return r.text

def measure(u):
    try:
        r=requests.get(u,headers={'User-Agent':UA,'Referer':'https://www.gemsny.com/'},timeout=30,stream=True);r.raise_for_status()
...
def main():
    ...
    sess=requests.Session();rows=[];cov=[];fails=[]
    for ix,row in enumerate(mine,1):
        ...
        try:
            html=fetch(sess,baseurl);first,text=extract_products(html,baseurl)
            ...
            if expected>0 and not first:raise RuntimeError(f'No SSR product cards on page 1; live_expected={live} baseline_expected={baseline} html_bytes={len(html)}')
            pages=max(1,math.ceil(expected/24)) if expected else max(1,as_int(row.get('expected_last_page'),1))
            products={};sets=[]
            for n in range(1,pages+1):
                h=html if n==1 else fetch(sess,page_url(baseurl,n));items,_=extract_products(h,page_url(baseurl,n));sets.append(frozenset(items))
            ...
            for k,v in products.items():
                ...
                cat.append({'category_name':name,'category_url':baseurl,'category_page_url':baseurl,'page_number':'SSR','sku':v['sku'],'product_key':k,'product_title':v['title'],'product_url':v['product_url'],'image_alt':v['title'],'card_family':'SSR_HTML_PRODUCT','image_url':iu,'original_url':iu,'original_width':w,'original_height':h,'original_status':status,'threshold_width':a.min_width,'threshold_height':a.min_height,'status':status,'compliance_basis':'ORIGINAL SOURCE','compliance_width':w,'compliance_height':h,'failure_reason':err if err else ('' if status=='PASS' else f'{w}x{h} below {a.min_width}x{a.min_height}'),'audit_mode':'REMAINING_SSR_HTML_RECOVERY_V5','audited_at':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())})
            rows.extend(cat);cov.append({'category_name':name,'category_url':baseurl,'status':'COMPLETE','expected_products_if_detected':expected,'unique_products_detected':len(products),'images_checked':len(cat),'failed_images':sum(x['status']=='FAIL' for x in cat),'image_errors':sum(x['status']=='ERROR' for x in cat),'note':f'Strict SSR HTML forced-page recovery; pages={pages}; live_expected={live}'})
