...
def main():
    ...
    base_img=rows(bimg);base_urls={r.get('category_url','') for r in base_cov if r.get('category_url')}
    ...
    for root in a.recovery_root:
        for cp in Path(root).rglob('coverage.csv'):
            ...
            im=find_image_file(cp.parent)
            label=str(cp.parent)
            complete=0
            for r in cr:
                u=r.get('category_url','')
                if u in base_urls and r.get('status')=='COMPLETE' and im:
                    chosen[u]=(r,cp.parent,label);complete+=1
            stats.append({'source':label,'coverage_rows':len(cr),'complete_rows':complete,'has_images':bool(im)})
    missing=sorted(base_urls-set(chosen));noncomplete=sorted(u for u,(r,_,_) in chosen.items() if r.get('status')!='COMPLETE')
    # Build images only for the source selected for each category.
    cache={};final_cov=[];final_img=[];mismatch=[]
    for u in sorted(base_urls):
        if u not in chosen:continue
        r,folder,label=chosen[u];final_cov.append(r)
        imgp=find_image_file(folder)
        if not imgp:continue
        k=str(imgp)
        if k not in cache:cache[k]=rows(imgp)
        rr=[x for x in cache[k] if x.get('category_url')==u]
        final_img.extend(rr)
    ...
    summary={'validation':'FAIL','rule':'source image width >= 500 AND height >= 500','categories_expected':700,'categories_complete':sum(r.get('status')=='COMPLETE' for r in final_cov),'missing_categories':missing,'noncomplete_categories':noncomplete,'image_count_mismatches':mismatch,'image_checks':len(final_img),'source_components':stats}
    if missing or noncomplete or mismatch or not final_img:
    ...
    for r in final_img:
        ...
        if iu:imgs.add(iu)
    summary.update({'validation':'PASS','categories_complete':700,'missing_categories':[],'noncomplete_categories':[],'image_count_mismatches':[],'category_product_assignments':len(prods),'unique_source_image_urls':len(imgs),'below_500x500':len(failed),'image_errors':len(errors),'issues_total':len(issues)})
